...
#     df.plt()
    ...
    lagged_correlation = pd.DataFrame.from_dict({x: [df['Watt-hour'].corr(df['Number_of_tweets_on_climatechange/energy'].shift(-t)) for t in range(x)] for x in df.columns})
